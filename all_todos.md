...
- [x] 7-D: Make receiver-first the universal argument convention — reordered `_create_stk` constructors (`GenVec_create_stk/_val_stk/_stk_arr`, `HashMap_create_stk`, `HashSet_create_stk`, `Queue_create_stk`, `matrix_create_stk`, `String_create_stk`) so the receiver/out-param is arg 1 instead of last; updated declarations + `nonnull` indices, all definitions, every call site (src/tests/examples), `VEC_STK`/`VEC_CX_STK` macros, and README ("receiver-last" → "receiver-first, always"). `Arena_*_stk` and the `matrix_generic.h` templates were already receiver-first, no change needed. Also fixed two unrelated pre-existing arg-order bugs found during the sweep: `ARENA_CREATE_STK_ARR` macro (args were in the wrong order vs. `Arena_create_arr_stk`'s signature) and `wcjson_create_from_file`'s `Arena_create_stk` call (that example target is commented out of CMakeLists, so it wasn't caught by the build). single_header/ intentionally left unregenerated (pre-existing, needs Python — see 5-G note). Verified: gcc build clean, 393 test assertions pass (only pre-existing ~86-leak ASan summary from HashSet/HashMap fixtures, noted at top of this file).
- [x] 7-E: Implement `String_ensure_null_term` (resolves the TODO above `TEMP_CSTR_READ` in `wc_string.h`) — writes `'\0'` at `buf[size]` without incrementing `size`, reusing `MAYBE_GROW_STR` (same growth path as `String_append_char`) so the write is always in freshly-owned memory rather than the SSO mode-flag byte at `stk[STR_SSO_SIZE-1]`, even when `size == capacity` exactly. Added `String_cstr_view` (const, never NULL for empty Strings — unlike `String_data_ptr`) as the intended read accessor. Rewrote `TEMP_CSTR_READ` to call `String_ensure_null_term` instead of append+pop; dropped `wctemp_cstr_read_cleanup`/`__attribute__((cleanup(...)))` entirely since size is never mutated, so there's nothing to undo. Updated `test_temp_cstr_read` (length assertion inside the block was testing the old append-based behavior) and added `test_temp_cstr_read_empty`, `test_ensure_null_term_direct`, `test_ensure_null_term_sso_boundary` (exact 23-char SSO→heap conversion case), `test_ensure_null_term_heap_boundary` (exact-capacity heap realloc case). 397/397 assertions pass, gcc build clean, no new ASan leaks (same pre-existing 88/836004312B).
